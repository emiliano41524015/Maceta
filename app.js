...
let userRole = roleSelector?.value ?? 'admin';
...
function normalizeHistoryEntry(entry = {}) {
  const timestamp = entry.timestamp ?? entry.fecha ?? new Date().toISOString();
  const sensores = {
...
function normalizeEvent(event = {}) {
  const timestamp = event.timestamp ?? event.fecha ?? new Date().toISOString();
...
function getFilteredModules() {
  ...
  if (activeFilter === 'alerts') {
    return modules.filter((module) => module.alertasActivas?.length);
  }
...
function updateSummary() {
  ...
  const alerts = modules.filter((module) => module.alertasActivas?.length).length;
  const humidityValues = modules
...
function handleModuleSubmit(formData) {
  ...
  if (editingModuleId) {
    modules = modules.map((module) => {
      ...
      updatedModule.historial = appendHistory(
        module.historial,
        createHistoryEntry(updatedModule, recalculatedAlerts)
      );
    ...
    newModule.historial = appendHistory(
      newModule.historial,
      createHistoryEntry(newModule, initialAlerts)
    );
